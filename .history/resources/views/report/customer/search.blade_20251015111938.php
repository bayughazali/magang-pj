...
<div class="container-fluid mt-4">
    ...
    onclick="deletePelanggan({{ $pelanggan->id ?? 0 }}, '{{ addslashes($pelanggan->nama_pelanggan ?? 'N/A') }}')"
    title="Hapus">
...
function deletePelanggan(id, nama) {
    console.log('Fungsi deletePelanggan terpanggil untuk ID:', id, 'Nama:', nama); // 👈 debug log
    Swal.fire({
...
<script>
function deletePelanggan(id, nama) {
    console.log("=== FUNGSI DELETE TERPANGGIL ===", id, nama); // DEBUG
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            .then(data => {
                console.log("RESPON SERVER:", data); // DEBUG
                Swal.fire('Berhasil', data.message || 'Data pelanggan dihapus!', 'success')
                    .then(() => location.reload());
            })
            .catch(error => {
