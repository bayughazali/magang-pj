...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Event listener untuk provinsi (gunakan Select2 event)
    provinsiSelect.on('change', function() {
        ...
        kabupatenSelect.trigger('change.select2');
    ...
    kabupatenSelect.on('change', function() {
        ...
        if (selectedProvinsi && selectedKabupaten && wilayahData[selectedProvinsi][selectedKabupaten]) {
            ...
            kecamatanList.forEach(kec => {
                kecamatanSelect.append(new Option(kec, kec));
            });
    ...
    $('.edit-provinsi').each(function() {
        ...
        // Load kabupaten & kecamatan saat modal dibuka
        $(`#editModal${reportId}`).on('shown.bs.modal', function() {
            const currentProvinsi = editProvinsi.val();
            const currentKabupaten = editKabupaten.find('option:selected').val();

            if (currentProvinsi && wilayahData[currentProvinsi]) {
                updateKabupatenEdit(editProvinsi[0], reportId, currentKabupaten);
            }
        });

        // Event change provinsi edit
        editProvinsi.on('change', function() {
            updateKabupatenEdit(this, reportId);
        });

        // Event change kabupaten edit
        editKabupaten.on('change', function() {
            updateKecamatanEdit(editProvinsi[0], this, reportId);
        });
    });
});

function updateKabupatenEdit(provinsiElement, reportId, selectedKabupaten = null) {
    const selectedProvinsi = provinsiElement.value;
    const kabupatenSelect = document.querySelector(`.edit-kabupaten-${reportId}`);
    const kecamatanSelect = document.querySelector(`.edit-kecamatan-${reportId}`);
    ...
    if (selectedProvinsi && wilayahData[selectedProvinsi]) {
        ...
        kabupatenList.forEach(kab => {
            ...
            if (selectedKabupaten && kab === selectedKabupaten) {
        ...
        // Load kecamatan jika ada selectedKabupaten
        if (selectedKabupaten) {
            updateKecamatanEdit(provinsiElement, kabupatenSelect, reportId);
        }
    }
}

function updateKecamatanEdit(provinsiElement, kabupatenElement, reportId) {
    const selectedProvinsi = provinsiElement.value;
    const selectedKabupaten = kabupatenElement.value;
    const kecamatanSelect = document.querySelector(`.edit-kecamatan-${reportId}`);
    const currentKecamatan = kecamatanSelect.querySelector('option:selected')?.value;
    ...
    if (selectedProvinsi && selectedKabupaten && wilayahData[selectedProvinsi][selectedKabupaten]) {
        ...
        kecamatanList.forEach(kec => {
            ...
            if (currentKecamatan && kec === currentKecamatan) {
                option.selected = true;
            }
